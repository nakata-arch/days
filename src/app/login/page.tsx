  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code === "auth/popup-closed-by-user" || error.code === "auth/cancelled-popup-request") {
        ...
        setErrorMessage(t("login.popupBlocked"));
      } else {
        setErrorMessage(t("login.loginFailed", { message: error.message }));
      }
      setLoading(false);
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3 pt-4">
            ...
            <p className="text-[11px] text-ink-faint leading-relaxed">
              {t("login.privacyAgreementBefore")}
              <a href="/privacy" className="underline underline-offset-2 text-[hsl(var(--accent))] hover:text-ink">{t("login.privacyAgreementLink")}</a>
              {t("login.privacyAgreementAfter")}
            </p>
          </div>
        </div>
